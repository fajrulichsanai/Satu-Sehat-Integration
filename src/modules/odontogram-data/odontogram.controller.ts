...
export class OdontogramController {
  ...
  async findOne(
    ...
    const data = await this.odontogramService.findByEncounter(
      encounterId,
      clinicId,
    );
  ...
  @Put()
  @ApiOperation({
    summary: 'Upsert odontogram (DMF-T calculated automatically)',
  })
  async upsert(
    @Param('encounterId', ParseIntPipe) encounterId: number,
    @ClinicId() clinicId: number,
    ...
    const data = await this.odontogramService.upsert(
      encounterId,
      clinicId,
      dto,
      user.userId,
