...
export class MedicationsService {
  ...
  async findAll(clinicId: number, query: MedicationQueryDto) {
    ...
    if (query.search) {
      qb.andWhere('(m.name LIKE :s OR m.genericName LIKE :s)', {
        s: `%${query.search}%`,
      });
    }
    ...
    if (query.nearExpiry) {
      qb.andWhere(
        'm.expirationDate IS NOT NULL AND m.expirationDate <= DATE_ADD(CURDATE(), INTERVAL 30 DAY)',
      );
    }
  ...
  async findOne(id: number, clinicId: number): Promise<Medication> {
    ...
    if (!med)
      throw new NotFoundException(`Obat dengan ID ${id} tidak ditemukan`);
    return med;
  }
  ...
  async adjustStock(
    ...
    return this.dataSource.transaction(async (manager) => {
      const med = await manager.findOne(Medication, {
        where: { id, clinicId },
  ...
  private toResponse(m: Medication) {
    ...
    return {
      ...
      isNearExpiry: m.expirationDate
        ? new Date(m.expirationDate) <= thirtyDays
        : false,
    };
  }
}
