    ...
    try {
      switch (resourceType) {
        case 'Patient': {
          ...
          fhirResource = FhirMapper.toFhirPatient(
            enc.patient,
            clinic.satusehatOrgId,
          );
          fhirPath = enc.patient.satusehatPatientId
            ? `Patient/${enc.patient.satusehatPatientId}`
            : 'Patient';
          break;
        }
        case 'Encounter': {
          const enc = await this.encounterRepo.findOne({
            where: { id: localId, clinicId },
            relations: { patient: true, location: true },
          });
          if (!enc)
            return { success: false, error: 'Encounter tidak ditemukan' };
          fhirResource = FhirMapper.toFhirEncounter(
            enc,
            clinic.satusehatOrgId,
            enc.location?.satusehatLocationId,
          );
          fhirPath = enc.satusehatEncounterId
            ? `Encounter/${enc.satusehatEncounterId}`
            : 'Encounter';
          break;
        }
        case 'Condition': {
          ...
          if (!diagnosis)
            return { success: false, error: 'Diagnosis tidak ditemukan' };
          const enc = await this.encounterRepo.findOne({
            where: { id: diagnosis.encounterId, clinicId },
          });
          if (!enc?.satusehatEncounterId)
            return { success: false, error: 'Encounter belum di-sync' };
          fhirResource = FhirMapper.toFhirCondition(
            diagnosis,
            String(enc.patientId),
            enc.satusehatEncounterId,
          );
          fhirPath = diagnosis.satusehatConditionId
            ? `Condition/${diagnosis.satusehatConditionId}`
            : 'Condition';
          break;
        }
        case 'Procedure': {
          const proc = await this.procedureRepo.findOne({
            where: { id: localId },
          });
          if (!proc)
            return { success: false, error: 'Prosedur tidak ditemukan' };
          const enc = await this.encounterRepo.findOne({
            where: { id: proc.encounterId, clinicId },
          });
          if (!enc?.satusehatEncounterId)
            return { success: false, error: 'Encounter belum di-sync' };
          fhirResource = FhirMapper.toFhirProcedure(
            proc,
            String(enc.patientId),
            enc.satusehatEncounterId,
          );
          fhirPath = proc.satusehatProcedureId
            ? `Procedure/${proc.satusehatProcedureId}`
            : 'Procedure';
          break;
        }
        case 'Observation': {
          const vs = await this.vitalSignRepo.findOne({
            where: { id: localId },
          });
          if (!vs)
            return { success: false, error: 'Vital sign tidak ditemukan' };
          const enc = await this.encounterRepo.findOne({
            where: { id: vs.encounterId, clinicId },
          });
          if (!enc?.satusehatEncounterId)
            return { success: false, error: 'Encounter belum di-sync' };
          fhirResource = FhirMapper.toFhirObservation(
            vs,
            String(enc.patientId),
            enc.satusehatEncounterId,
          );
        ...
        case 'MedicationRequest': {
          ...
          const enc = await this.encounterRepo.findOne({
            where: { id: rx.encounterId, clinicId },
          });
          if (!enc?.satusehatEncounterId)
            return { success: false, error: 'Encounter belum di-sync' };
          fhirResource = FhirMapper.toFhirMedicationRequest(
            rx,
            String(enc.patientId),
            enc.satusehatEncounterId,
            clinic.satusehatOrgId,
          );
          fhirPath = rx.satusehatMedreqId
            ? `MedicationRequest/${rx.satusehatMedreqId}`
            : 'MedicationRequest';
          break;
        }
        default:
      ...
      await this.saveLog(
        ...
        fhirResource,
        data,
        success ? undefined : JSON.stringify(data),
      );
