...
export class ProceduresService {
  ...
  async create(
    ...
    if (encounter.status === EncounterStatus.FINISHED) {
      throw new ForbiddenException(
        'Tidak dapat menambah prosedur pada encounter yang sudah selesai',
      );
    }

    if (dto.toothNumber && !VALID_TOOTH_NUMBERS.has(dto.toothNumber)) {
      throw new BadRequestException(
        `Nomor gigi '${dto.toothNumber}' tidak valid (gunakan notasi FDI)`,
      );
    }
  ...
  async remove(
    ...
    if (encounter.status === EncounterStatus.FINISHED) {
      throw new ForbiddenException(
        'Tidak dapat menghapus prosedur pada encounter yang sudah selesai',
      );
    }
    ...
    if (!procedure) {
      throw new NotFoundException(
        `Prosedur dengan ID ${procedureId} tidak ditemukan`,
      );
    }
